...
const statusBadgeStyles: Record<string, string> = {
  ...
  Researching: 'border-blue-500/40 bg-blue-950/30 text-blue-300',
  Planned: 'border-[#26211B] bg-[#0A0908] text-[#8C6D4F]',
};

export const EngineeringLabSection: React.FC = () => {
  const { labTracks: contextTracks } = usePortfolio();
  const tracks = (contextTracks || []).filter((t) => t.visible !== false);
  const [activeTrackState, setActiveTrackState] = useState<LabTrack | null>(null);

  const activeTrack = activeTrackState || tracks[0] || {
    id: 'track-1',
    stepNumber: '01',
    category: 'Backend & AsyncIO',
    title: 'High-Throughput Python Microservices',
    status: 'Completed',
    objective: 'Building production REST APIs with FastAPI, AsyncIO, and Redis caching layers.',
    architecture: 'FastAPI -> Redis Cache -> PostgreSQL -> Pydantic V2',
    keyLearnings: [
      'Asynchronous request pipelining with uvloop',
      'Distributed rate limiting and token bucket algorithms',
      'Database connection pooling with SQLAlchemy 2.0 Async',
    ],
    technologies: ['Python 3.12', 'FastAPI', 'Redis', 'PostgreSQL', 'Docker'],
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-1/4 left-1/3 w-[35rem] h-[35rem] bg-[#D4AF37]/5 rounded-full blur-[160px] pointer-events-none" />
      ...
      <div className="max-w-[1760px] mx-auto w-full relative z-10">
        ...
        >
          <span className="text-[10px] sm:text-[11px] font-mono font-medium tracking-[0.25em] sm:tracking-[0.3em] uppercase text-[#D4AF37]">
            04 / ENGINEERING LAB &amp; ARCHITECTURE LAB
          </span>
        ...
        >
          <div>
            <h2 className="text-3xl sm:text-5xl lg:text-6xl font-serif text-white leading-[1.08] tracking-tight">
              Systems Progression <span className="italic text-[#D4AF37]">Lab.</span>
            </h2>
          </div>
